...
joint1publisher = rospy.Publisher(
    '/rrbot/joint1_position_controller/command', Float64, queue_size=10)
joint2publisher = rospy.Publisher(
...
def rrarm_positions_publisher():
    ...
    while not rospy.is_shutdown() and positioning:
        rrarm_initial_joints_position(
            joint1initial_position, joint2initial_position)
        rospy.sleep(1)
        ...
        rospy.sleep(1)
        joint1current_position = rrarm_lift_up(
            joint1current_position, step_upward1)
        positioning = False
